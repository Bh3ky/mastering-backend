...
class TestPostViewSet:
    ...
    def test_update(self, client, user, post):
        # test method for post updating
        client.force_authenticate(user=user)
        data = {
            "body": "Test Post Body",
            "author": user.public_id.hex
        }
        response = client.put(self.endpoint + str(post.public_id) + "/", data, format="json")

        assert response.status_code == status.HTTP_200_OK
        assert response.data['body'] == data['body']

    def test_delete(self, client, user, post):
        # test method for post deletion
        client.force_authenticate(user=user)
    ...
    def test_updated_anonymous(self, client, post):
        data = {
            "body": "Test Post Body",
            "author": "test_user"
        }
